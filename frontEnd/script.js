...
// função para enviar a mensagem do utilizador e lidar com a resposta em streaming do chatbot
async function enviarMensagem() {
  ...
  if (mensagem.length > 500) {
    adicionarMensagemBot(
      "❌ Mensagem demasiado longa (máximo 500 caracteres).",
    );
    return;
  }
  ...
  // abrir conexão SSE para receber a resposta em streaming do chatbot
  const eventSource = new EventSource(
    BASE_URL + "/api/chat?message=" + encodeURIComponent(mensagem),
  );
  ...
  eventSource.onmessage = function (evento) {
    if (evento.data === "[DONE]") {
      ...
      // chamar a rota de function calling para executar a ação solicitada pelo chatbot
...
// funções para adicionar mensagens à interface do chatbot e do utilizador
function adicionarMensagemBot(texto) {
  ...
  div.innerHTML =
    '<span class="icone-bot">🤖</span><div class="texto-mensagem">' +
    texto +
    "</div>";
...
// função para adicionar mensagens do utilizador à interface
...
// função para carregar os itinerários da base de dados e mostrar na interface
async function carregarItinerariosDaBD() {
  const resposta = await fetch(BASE_URL + "/api/acao");
  ...
  for (const viagemId in viagens) {
    ...
    let html =
      '<div class="cartao-viagem" id="viagem-' +
      viagemId +
      '">' +
      '<div class="cabecalho-viagem">✈️ ' +
      viagem.nome +
      '<button class="btn-apagar" onclick="apagarViagem(' +
      viagemId +
      ')">🗑️ Apagar</button>' +
      "</div>";
    ...
    for (let i = 0; i < viagem.dias.length; i++) {
      ...
      html +=
        '<div class="cartao-dia" id="item-' +
        dia.id +
        '">' +
        '<div class="cabecalho-dia">📅 Dia ' +
        dia.dia +
        "</div>" +
        ...
        dia.local_nome +
        "</div>" +
        '<div class="local-transporte">🚌 ' +
        dia.transporte +
        "</div>" +
        '<div class="local-descricao">' +
        dia.descricao +
        "</div>" +
        '<button class="btn-apagar" onclick="apagarItem(' +
        dia.id +
        ')">🗑️ Remover</button>' +
        "</div>" +
        "</div>";
...
// função para apagar uma viagem e todos os seus dias
async function apagarViagem(id) {
  if (
    !confirm(
      "Tens a certeza que queres apagar esta viagem e todos os seus dias?",
    )
  )
    return;

  try {
    const resposta = await fetch(BASE_URL + "/api/acao/" + id, {
      method: "DELETE",
    });
...
// função para apagar um dia do itinerário
async function apagarItem(id) {
  if (!confirm("Tens a certeza que queres remover este dia?")) return;

  try {
    const resposta = await fetch(BASE_URL + "/api/acao/dia/" + id, {
      method: "DELETE",
    });
    if (!resposta.ok) throw new Error();
...
// função para limpar a conversa e o histórico do chat
async function limparConversa() {
  ...
  await fetch(BASE_URL + "/api/chat/limpar", { method: "DELETE" });

  document.getElementById("area-mensagens").innerHTML = "";
  adicionarMensagemBot("Conversa reiniciada 🔄 Para onde queres viajar?");
}

//  função para carregar o histórico do chat da base de dados e mostrar na interface
